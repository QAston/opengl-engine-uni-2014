...
int main(int argc, char **argv) {
  ...
  shared_ptr<Cube::Input> cubeSoundInput =
      make_shared<Cube::Input>(nullptr, cubeSound.get());
  ...
  shared_ptr<FPSCamera::Input> cameraInput =
      make_shared<FPSCamera::Input>(camera);
  ...
  shared_ptr<Mover> collidingMover1 =
      make_shared<Mover>(wobj1.get(), 'w', 's', 'a', 'd');

  shared_ptr<Mover> collidingMover2 =
      make_shared<Mover>(wobj2.get(), 'i', 'k', 'j', 'l');
...
ALuint loadAudioFile(const char *fileName) {
  std::string s = resourcePath(std::string("/sound/") + fileName);
  fileName = s.c_str();
  ALenum error;
